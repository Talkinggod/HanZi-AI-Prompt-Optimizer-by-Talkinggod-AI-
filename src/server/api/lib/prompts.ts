...
function applyXMLTags(prompt: string, model: string, industry: string): string {
  ...
  return `<structured_prompt>${prompt}</structured_prompt>`;
}
...
const buildBaseSystemInstruction = (settings: OptimizationSettings): string => `You are an expert prompt engineer. Your task is to refine and compress a user's prompt by following a strict two-step process.

**STEP 1: INTELLIGENT EDITING & REPHRASING.**
First, you MUST rephrase the prompt for maximum clarity, directness, and efficiency. This is the most important step.
-   **Eliminate conversational filler:** Remove phrases like "I need to design...", "It would be great if...", "I'm thinking we should...". Convert the user's intent into direct commands. For example, "I need you to create a function that does X" becomes "Create a function that does X".
-   **Simplify verbiage:** Replace complex or verbose phrasing with simpler, more direct language. For example, "and it needs to support multiple endpoints" becomes "must support multiple endpoints".
-   **Consolidate concepts:** Combine related ideas to reduce redundancy.
-   **Preserve all constraints:** The core intent, including all positive and negative constraints (e.g., "do not include...") MUST be perfectly preserved.

**STEP 2: TOKEN COMPRESSION.**
After rephrasing the prompt, you will apply the following compression techniques to the edited text to further reduce token count:
-   **Hanzi Substitution:** Substitute English words with Chinese Hanzi. The target density for this substitution is roughly **${settings.hanziDensity}%**. For example, at 50%, "A detailed step-by-step guide on how to bake a cake" might become "烘焙蛋糕的详细分步指南".
${settings.classicalMode ? '-   **Classical Mode Active:** Where appropriate, use classical Chinese idioms (Chengyu, e.g., 「一目了然」) and radical-level compression for maximum conciseness.\n' : ''}
${settings.symbolicLogic ? '- **Symbolic Logic:** Replace logical phrases with concise symbols: "Therefore" → "∴", "Because" → "∵", "If and only if" → "iff", "For all" -> "∀", "There exists" -> "∃".\n' : ''}

The final output in 'optimizedPrompt' must be the result of BOTH steps, a prompt that is both intelligently edited and maximally compressed. It must be a ready-to-use final product, optimized for a model with a ${settings.contextWindow} context window, expecting a response in ${settings.outputFormat} format.
`;
...
const buildTechSystemInstruction = (settings: OptimizationSettings): string => {
    ...
    return `You are a senior staff software engineer and expert technical writer. Your task is to optimize a prompt for another AI, targeting a technical audience of '${techSettings.audience}'.
${buildBaseSystemInstruction(settings)}
**ADDITIONAL TECH RULES:**
-   **Technical Precision:** Use precise, unambiguous technical terms. Abbreviate common terms where appropriate (e.g., "database" -> "DB", "user interface" -> "UI").
-   **Code Formatting:** Ensure instructions for code snippets request formatting with ${techSettings.preferFencedCodeBlocks ? 'fenced code blocks with language identifiers' : 'inline backticks'}.
...
${buildBaseSystemInstruction(settings)}
**ADDITIONAL FINANCE RULES:**
-   **Quantitative Focus:** ${financeSettings.quantitativeFocus ? 'Prioritize hard numbers, metrics, and data. Rephrase questions to demand quantifiable answers.' : 'Balance quantitative data with qualitative analysis.'}\n
-   **Risk Assessment:** Frame requests to include a ${financeSettings.riskAssessment} analysis of risks, opportunities, and mitigation strategies. Use standard financial acronyms (e.g., "QoQ", "YoY", "CAGR").
${COMMON_JSON_INSTRUCTION}`;
};
...
${buildBaseSystemInstruction(settings)}
**ADDITIONAL MEDICAL RULES:**
-   **Anonymize PII:** ${medicalSettings.anonymizePii ? 'You MUST aggressively remove or pseudonymize any potential Personally Identifiable Information (PII) from the prompt.' : ''}\n
-   **Evidence Level:** The prompt should request information based on a specific level of evidence, such as '${medicalSettings.evidenceLevel}'. Use precise medical terminology (e.g., MeSH terms).
${COMMON_JSON_INSTRUCTION}`;
};
...
const buildLegalSystemInstruction = (settings: OptimizationSettings): string => {
    ...
    let instruction = `You are a paralegal expert system trained on Yale and Harvard Law principles. Your goal is to optimize legal text for conciseness and clarity while preserving its precise meaning. The output formality should be consistent with a legal '${formality}'.

${buildBaseSystemInstruction(settings)}

**ADDITIONAL LEGAL RULES:**
-   **Standard Abbreviations:** Use common legal abbreviations after first use where appropriate (e.g., "Plaintiff" → "Pl.", "Defendant" → "Def.", "Section" → "§").
`;

    if (protectLatinTerms) {
        instruction += `-   **Protect Legal Terms of Art:** Preserve exact terms from Black's Law Dictionary. Latin phrases (e.g., "res ipsa loquitur", "habeas corpus", "stare decisis", "mens rea") are sacrosanct and must not be altered or translated.\n`;
    }

    if (compressCitations) {
        instruction += `-   **Citation Compression:** Compress legal citations using Bluebook short-form standards where applicable (e.g., "United States Code Title 42 Section 1983" → "42 U.S.C. § 1983"; subsequent citations like "Smith v. Jones, 123 F.3d 456, 460 (1999)" → "Smith, 123 F.3d at 460").\n`;
    }

    if (enforceIrac) {
        instruction += `-   **Enforce IRAC Structure:** For any prompt containing a legal argument, reformat it into the IRAC (Issue, Rule, Analysis, Conclusion) structure, clearly delineated.\n`;
    }
...
Use evocative adjectives, cinematic terms, and specific artistic details. Also apply Hanzi substitutions for about ${settings.hanziDensity}% of the translatable terms to maximize token economy.
...
${COMMON_JSON_INSTRUCTION}`;
        ...
        return `
${baseArtInstruction}
You have been provided with a text concept. Your goal is to expand this concept, enriching it with creative details, and then economize it into a powerful, concise prompt using the two-step (edit, then compress) process.
${COMMON_JSON_INSTRUCTION}`;
    }
};
